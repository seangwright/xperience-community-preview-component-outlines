...
public class OutlinesStylesTagHelperComponentTests
{
    ...
    public async Task The_Styles_Element_Will_Not_Be_Added_When_The_Request_Is_In_Live_Mode()
    {
        ...
        var tagHelperContext = new TagHelperContext("head", [], new Dictionary<object, object>(), Guid.NewGuid().ToString());
        var output = new TagHelperOutput("head", [], (val, encoder) => Task.FromResult<TagHelperContent>(new DefaultTagHelperContent()));
